...
export class QeSectionHeadApproveComponent implements OnInit {
  ...
  config_auth = 'qe_department_head'
  userApproveList: any = [];
  dateNow!: Date
  constructor(
  ...
  async updateRequest(confirmValue: any) {
    this.request.status = 'qe_section_head';
    this.request.step4.push({
      access: 'qe_department_head',
    ...
    await this._request.updateRequest_form(this.request._id, this.request).toPromise();
    await (await this.$share.insertLogFlow('qe_section_head', this.request.step1.controlNo, confirmValue, this.userLogin)).toPromise();
    this._toast.success();
    setTimeout(() => {
      this.router.navigate(['/approve']);
    }, 500);
  ...
  findMe(step4: any) {
    return new Promise((resolve, reject) => {
      const resultFind = step4.find((u: any) => u.access == 'qe_section_head')
      if (resultFind) {
        resolve(resultFind)
      } else {
