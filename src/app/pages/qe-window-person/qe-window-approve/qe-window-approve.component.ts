...
export class QeWindowApproveComponent implements OnInit {
  ...
  async updateRequest(confirmValue: any) {
    this.request.status = 'qe_window_person';
    ...
    await this._request.updateRequest_form(this.request._id, this.request).toPromise();
    await (await this.$share.insertLogFlow('qe_window_person', this.request.step1.controlNo, confirmValue, this.userLogin)).toPromise();
    this._toast.success();
    setTimeout(() => {
      this.router.navigate(['/approve']);
    }, 500);
  ...
  findMe(step4:any){
    return new Promise((resolve,reject)=>{
      const resultFind = step4.find((u:any)=> u.access =='qe_window_person')
      if(resultFind){
        resolve(resultFind)
      }else{
